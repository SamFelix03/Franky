...
class ZerePyCLI:
    ...
    def _initialize_commands(self) -> None:
        ...
        self._register_command(
            Command(
                ...
                tips=["Type 'exit' to end the chat session"],
                handler=self.chat,
                aliases=['talk']
            )
        )
        ...
        self._register_command(
            Command(
                name="ollama-chat",
                description="Start a direct chat session with Ollama, bypassing the agent's personality",
                tips=["Type 'exit' or 'quit' to end the chat session",
                      "This uses the Ollama model configured in your agent"],
                handler=self.ollama_chat,
                aliases=['ollama', 'ochat']
            )
        )
    ...
    def chat(self, input_list: List[str]) -> None:
        """Handle chat command"""
        if self.agent is None:
    ...
    def ollama_chat(self, args: List[str]) -> None:
        """Start a direct chat session with Ollama"""
        if not self.agent:
            logger.error("\n❌ No agent loaded. Use 'load-agent' to load an agent first.")
            return
            
        # Check if Ollama connection is configured
        if "ollama" not in self.agent.connection_manager.connections:
            logger.error("\n❌ Ollama connection not found in agent configuration.")
            return
            
        if not self.agent.connection_manager.connections["ollama"].is_configured():
            logger.error("\n❌ Ollama connection is not configured. Use 'configure-connection ollama' first.")
            return
            
        # Execute the ollama-chat action
        from src.action_handler import execute_action
        execute_action(self.agent, "ollama-chat")
